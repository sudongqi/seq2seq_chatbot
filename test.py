...
reader = s2s_reader.reader(file_name = file_name, batch_size = batch_size, buckets=buckets, bucket_option = bucket_option, signal=signal)
...
for i in xrange(num_layers):
	single_cell = tf.nn.rnn_cell.LSTMCell(num_units=recurrent_size, num_proj=hidden_size, state_is_tuple=True)
...
#output layers
project_w = tf.Variable(tf.truncated_normal([hidden_size, embedding_size], stddev=0.1), name="project_w")
